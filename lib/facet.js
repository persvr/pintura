...
var SchemaControlled = function(facetSchema, sourceClass, facetResolver){
	...
	return function wrap(source, wrapped, partial){
		...
		var instancePrototype = Object.create(facetPrototype, {
			load: {
				value: function(){
					if(facetSchema.allowed && !facetSchema.allowed(facetResolver.env, source)){
...
exports.canFacetBeAppliedTo = canFacetBeAppliedTo;
function canFacetBeAppliedTo(schema, store){
...
exports.SchemaFacet = function(schema){
	function FacetForStore(sourceStore, facetResolver){
		if(!canFacetBeAppliedTo(schema, sourceStore)){
		...
		facetedStore.forStore = FacetForStore;
		facetedStore._nonTransactionalFacet = nonTransactionalFacet;
		return facetedStore;
	}
	var nonTransactionalFacet = FacetForStore(schema.appliesTo, null);
	nonTransactionalFacet._nonTransactionalFacet = nonTransactionalFacet;
	return Facet(nonTransactionalFacet);
};
...
function rpcInvoke(target, rpc){
	try{
		var result = target[rpc.method].apply(target, rpc.params);
