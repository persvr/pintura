...
var DatabaseError = require("./errors").DatabaseError,
	...
	extendSome = require("./util/lazy").extendSome,
			...
			load: {
				value: function(){
					...
					if(source.load && this != source){
						...
						source = sourceClass.get(sourceClass.getId ? sourceClass.getId(source) : source.id);
					}
					copyFromSource();
					loaded();
					return this;
			...
			defineProperties(instancePrototype,{
				...
				set: {
					value: function(name, value){
						...
						sourceClass.get(sourceClass.getId ? sourceClass.getId(source) : source.id).set(name, value);
						this[name] = value;
					},
					enumerable: false
				},
				save: {
					value: function(id){
						...
						try{
							...
							if(newIndex > -1){
								transaction.generatedId = sourceClass.getId ? sourceClass.getId(source) : source.id;
