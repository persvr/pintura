...
var defineProperties = require("./util/es5-helper").defineProperties;
...
function FacetedStore(store, facetClass, wrap){
	...
	constructor.create= function(props){
		var instance = this.wrap({}, this.transaction, null, NEW);
		for(var i in props){
			instance[i] = props[i];
		}
...
function rpcInvoke(target, rpc){
	try{
		...
		if(target[rpc.method] || !target.__noSuchMethod__){
			...
			result = target.__noSuchMethod__.call(target, rpc.method, rpc.params);
		}
