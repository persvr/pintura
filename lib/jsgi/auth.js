...
exports.Authentication = function(security, nextApp){
	...
	return function(request){
		...
		if(authorization){
			...
			user.resolve(security.authenticate(userPass[0], userPass[1]));
		}
		else{
...
function dir(){var sys=require('sys');for(var i=0,l=arguments.length;i<l;i++)sys.debug(sys.inspect(arguments[i]));}
						when(security.getAuthClass().get(cookieId), function(auth){
							// TODO: check auth record expiry
//dir('AUTH:', auth, cookieId);
							if (auth && auth.user) {
								when(security.getUserClass().get(auth.user), function(u){
//dir('USER:', u);
									if (u) {
										user.resolve(u);
									} else {
...
exports.setAuthCookie = function(authId, username, expires){
	...
	if(!authId){
		...
		// negative "expires" means number of days after the current moment
		if (expires < 0)
			expires = new Date((new Date().valueOf())-expires*86400*1000);
