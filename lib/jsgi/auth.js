...
exports.Authentication = function(security, nextApp){
	...
	return function(request){
		...
		var user = require("promised-io/promise").defer();
...
function cookieVerification(request){
	// TODO: our cookie ID can be not only \w -- what alphabet sha1() uses?!
...
exports.setAuthCookie = function(authId, username, expires){
	...
	// N.B. http://www.owasp.org/index.php/HttpOnly
	response.headers = {
		"set-cookie": "pintura-auth=" + authId + ";path=/;HttpOnly" + (expires ? ";expires=" + expires.toUTCString() : ""),
		username: username
	};
	response.body = [];
	response.status = 200;
