...
function MediaConverter(mediaSelector, nextApp){
	return function(request){
		if(METHOD_HAS_BODY[request.method.toLowerCase()]){
			...
			request.input = requestMedia.deserialize(request.input, request.store, request);
		}
		...
		if(response.body === undefined){
			...
			response.headers["content-type"] = responseMedia.mediaType + "; charset=UTF-8; definedby=../Class/" + (request.store && request.store.id);
