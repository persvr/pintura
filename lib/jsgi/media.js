...
//dir('RESP', request.nodeRequest.url, request.headers);
...
//				return {status: 406, headers: {}, body:["The Accept header did not contain an acceptable media type"]};
			...
			if (!headers["content-type"]) {
				...
				if(schema && schema.schema){
					headers["content-type"] += "; profile=../Class/" + schema.schema.getId(schema);
				}
				response.body = responseMedia.serialize(body, request, response);
			}
			return response;
		});
	};
