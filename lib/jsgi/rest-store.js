		...
		if(method in METHOD_HAS_BODY){
			if(!store[method]){
				...
				if(method === "get" && request.pathInfo.substring(request.pathInfo.length - 1) === "/"){
					...
					var start = 0;
					var limit = Infinity;
					// absent or invalid "Range:" results in no limits
					if (metadata.range) {
						var range = metadata.range.match(/^items=(\d+)-(\d+)?/);
						if (range) {
							start = +range[1] || 0;
							limit = ((+range[2] || Infinity) + 1 - start);
						}
					}
					// N.B. we forcibly limit the range by a configurable number returned by Facet.maxQueryLimit
					var maxQueryLimit = store.maxQueryLimit;
					if (maxQueryLimit) {
						var hardLimit = typeof maxQueryLimit === "function" ? maxQueryLimit() : maxQueryLimit;
						if (hardLimit < limit)
							limit = hardLimit;
					}
					metadata.range = {start: start, limit: limit, calculateTotalCount: Boolean(range || hardLimit !== undefined)};
					// queries are not decoded, the info needs to be retained for parsing
					responseValue = store.query(queryString, metadata);
					if (metadata.range.calculateTotalCount) {
						// we have to wait for promise for counts to be set (e.g., mongo)
						responseValue = when(responseValue, function(responseValue){
							if (metadata['x-jqgrid'])
								responseValue = {rows: responseValue.toRealArray(), page: 1, total: 1, records: responseValue.length};
