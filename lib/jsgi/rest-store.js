			...
			if(!store[method]){
				if(store.__noSuchMethod__){
					if(!METHOD_HAS_BODY[method]){
						responseValue = store.__noSuchMethod__(method, [decodeURIComponent(path)]);
					}
					else{
						responseValue = store.__noSuchMethod__(method, [request.input, decodeURIComponent(path)]);
					}
				}
				else{
					status = 405;
				...
				if(method === "get" && request.headers.range){
					...
					responseValue = store[method](decodeURIComponent(path));
				}
			}
			else{
				// call the store with the request body and the path
				responseValue = store[method](request.input, decodeURIComponent(path));
