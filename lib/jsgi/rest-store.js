			...
			if(!store[method]){
				...
				if(method === "get"){
					if(request.pathInfo.substring(request.pathInfo.length - 1) === "/"){
						...
						// N.B. this is the right place to honor store.limit
						// TODO:
						// ...
						...
						if (metadata.range) {
							...
							if (range) {
								...
								if (end >= start) {
									queryString = "and(" + queryString + ")&limit(" + (end + 1 - start)+ "," + start + ",Infinity)";
								}
							}
						}
						...
						if(range){
							...
							when(responseValue, function(responseValue){
								when(responseValue.totalCount, function(count){
