				...
				if(method === "get"){
					if(request.pathInfo.substring(request.pathInfo.length - 1) === "/"){
	                    var queryString = request.queryString.replace(/\?.*/,''),
	                        defaultLimit = store.defaultLimit || settings.defaultLimit,
	                        maxLimit = store.maxLimit || settings.maxLimit || Infinity;
	                    if (metadata.range || defaultLimit || maxLimit) {
							// invalid "Range:" are ignored
							var range = metadata.range && metadata.range.match(/^items=(\d+)-(\d+)?$/),
	                            start = 0,
	                            end;
	                        if (range) {
								start = +range[1] || 0;
								end = range[2];
								if (end !== undefined) end = +end; else end = Infinity;
	                            if (defaultLimit && end + 1 - start > defaultLimit) {
	                                end = start + defaultLimit - 1
	                            }
							}
	                        else {
	                            end = defaultLimit - 1;
	                        ...
	                        if (end >= start) {
	                            metadata.range = "items=" + start + "-" + (end !== Infinity && end || "")
	                            queryString += "&limit(" + (end + 1 - start)+ "," + start + "," + maxLimit + ")";
	                        }
						}
