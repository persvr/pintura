				...
				if(method === "get" && request.pathInfo.substring(request.pathInfo.length - 1) === "/"){
					...
					if (metadata.range) {
						...
						if (!range) delete metadata.range;
							...
							metadata.range = {
								start: +range[1] || 0,
								end: range[2] ? +range[2]+1 : Infinity, // slice is exclusive
					...
					if(metadata.range){
						...
						responseValue = when(responseValue, function(responseValue){
							return when(responseValue.totalCount, function(count){
								var end = metadata.range.start + (responseValue.length || 0) - 1;
								headers["content-range"] = "items " + metadata.range.start + '-' + end + '/' + (count || '*');
								status = (metadata.range.start === 0 && count -1 === end) ? 200 : 206;
