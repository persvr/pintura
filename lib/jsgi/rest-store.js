			...
			while(!model[method]){
				...
				if(slashIndex > -1 && model[part = path.substring(0, slashIndex)]){
					...
					if(model[path]){
						...
						status = 301;
						headers.location = request.scheme + "://" + request.headers.host + request.scriptName + request.pathInfo + '/';
						responseValue = request.pathInfo + " does not define " + method + " but a model exists at " + headers.location;
					}
					else if(method == "get"){
						status = 404;
						responseValue = (part || path) + " not found as a sub-model for " + model.id;
					}
					else{
						status = 405;
						var methods = [];
...
dir('RV?:', responseValue, responseValue instanceof Response);
		return when(responseValue, function(responseValue){
dir('RV!:', responseValue, responseValue instanceof Response);
			if(responseValue instanceof Response || (responseValue && responseValue._jsgiResponse_)){
				return responseValue;
			}
			return {
				status: status,
