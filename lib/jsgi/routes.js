	...
	var handler = function(request){
		function toHTML(){
		    return {
				status: 200,
				headers: {},
				body: '<form method="post" action="' + (form.uri||'') + '">'+form.toHTML()+'<input type="submit" /></form>'
			};
		}
		var response = require('promised-io/promise').defer();
		if (request.method === 'POST') {
			form.handle(request.body || {}, {
				success: function(form){
					// do something with collected data
					when(success(form.data),
						// redirect to index
						function(){
							response.resolve({status: 301, headers: {'content-type': 'text/plain', location: '/index.html'}, body: []});
						},
						function(){
							response.resolve(toHTML());
						}
					);
				},
				other: function(form){
