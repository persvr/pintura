	...
	return function(request){
		request.getSession = function(createIfNecessary, expires){
			...
			if(cookieId){
				...
				return when(exports.getSessionModel().get(cookieId), function(session){
					if (session) request.nonce = session.nonce;
				});
				...
				return exports.getSessionModel().put({
					...
					nonce: exports.generateSessionKey()
				},{
					id: newSessionId
				});
			}
		...
		return when(nextApp(request), function(response){
			...
			var session = exports.currentSession();
			// regenerate session nonce
			session.nonce = exports.generateSessionKey();
			exports.getSessionModel().put(session, {id: session.id});
			// no session at upper layers
			delete exports.currentSession;
			return response;
		});
	};
};
