...
exports.Templated = function(options, nextApp){
	...
	media.Media({
		...
		serialize: function(object, request, response){
			...
			return [jade.partial('list', {items: arr, entity: template, options: options})];
		}
	});
	...
	jade.include = function(template){
		return require('promised-io/fs').read(options.root + '/' + template + '.haml');
	}
	...
	jade.partial = function(template, locals){
		...
		return jade.render(html, {
			scope: jade, // N.B. this will be 'this' in templates
