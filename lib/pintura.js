...
function JsgiApp(nextApp, config){
		...
		// First we use the request as the context (preserved across promises)
	return require("jsgi/context").Context(
											...
											require("jsgi/transactional").Transactional(config.database,
												...
												require("./jsgi/auth").Authentication(config.security,
													...
													require("./jsgi/faceted").FacetAuthorization(config.facetSelector,
														// Comet notifications
														require("./jsgi/comet").Notifications(config.notificationsPath,
															// Add and retrieve metadata from objects
															require("./jsgi/metadata").Metadata(
																// Final REST handler
