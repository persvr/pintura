...
function JsgiApp(nextApp, config){
		...
		return require("./jsgi/cache").UpdateCache(config.responseCache,
									...
									require("./jsgi/error").ErrorHandler(
										...
										require("perstore/jsgi/transactional").Transactional(config.database,
											...
											require("./jsgi/auth").Authentication(config.security,
												require("./jsgi/cache").FetchCache(config.responseCache, 
...
eports.addConnection = exports.app.addConnection = function(connection){
	Connector("local-workers", connection);
	connection.observe("message", function(message){
		if(message.method !== "subscribe"){
			exports.directApp(JSON.parse(message));
		}
	});
};
