...
function Faceted(FacetResolver, nextApp){
	return function(env){
		...
		env.scriptInfo = parts[2];
		env.facet = 
			(env.facetedTransaction = FacetResolver(env))
				.getEntityStore(parts[1]);
		return nextApp(env);
...
function MediaConverter(media, nextApp){
	return function(env){
		...
		var response = nextApp(env);
		print("unserialized body" + response.body);
	...
	return function(env){
		...
		if(method in NO_BODY_METHODS){
			if(method === "get" && env.headers.range){
				...
				responseValue = facet.query(path + (env.queryString ? '?' : '') + env.queryString, {start: start, end: end});
				headers["content-range"] = "items " + start + '-' + end + '/' + responseValue.totalCount;
				status = 206;
			}
			else{
