...
var parseQuery = exports.parseQuery = function(/*String*/query){
	...
	if(query.replace(/([&\|,])?(([\+\*\-:\w%\._]+)(=|=?<|=?>|!=|=[a-z]*=)([\+\*\-:\w%\._]+)|(([\+\*\-:\w%\._]+)(\(?))|(\))|(.+))/g,
		 //      <-delim-> <--- name ---------  comparator   ----- value  ---->|<-method/value--openParan->|<-closedParan->|<-illegalCharacter->
		function(t, termDelimiter, expression, name, comparator, value, call, methodOrValue, openParan, closedParan, illegalCharacter){
			if(comparator){
				comparison = {
					type:"comparison", 
				...
				if(openParan){
					...
					var callNode = {
						type:"call", 
						parameters:[],
						name: convertPropertyName(methodOrValue),
						logic: termDelimiter
					};
					terms.push(callNode);
...
exports.executeQuery = function(query, options, target){
	...
	query.forEach(function(term){
		if(term.type == "comparison"){
			...
			if(methods[term.method]){
				var index = (options.parameters = options.parameters || []).push(term.parameters);
				js += "target = methods." + term.method + ".apply(target,options.parameters[" + (index -1) + "]);";
			}
			else{
				throw new URIError("Invalid JSON Query syntax, " + term.method + " not implemented");
			}
		}
		else{
