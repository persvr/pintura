...
exports.DefaultSecurity = function(){
			...
			function (user){
				...
				// logout -> rip the session, session key cookie
					...
					if (user && user.getSession) {
						...
						//callMethod(security.getAuthClass(), 'delete', [user.getSession(), true]);
						security.getAuthClass().store['delete'](user.getSession());
					}
					// FIXME: should be cron job?
					security.validateSessions();
				}
				return response;
			});
	}
	...
	var security = {
		...
		validateSessions: function(){
			var criteria = require('perstore/resource-query').Query('le(expires,$1)', [(new Date()).toISOString()]);
			when(security.getAuthClass().query(criteria), function(deadSessions){
				//dir('CORPS:', deadSessions);
				deadSessions.forEach(function(session){
					//dir('CORPS:', session);
					// N.B. delete() is not found in Model()
					//callMethod(security.getAuthClass(), 'delete', [session.id]);
					security.getAuthClass().store['delete'](session.id);
				});
			});
		},
		getAllowedFacets: function(user, request){
			...
			if((settings.admins && settings.admins.indexOf(user.id) > -1) || settings.bypassSecurity){
				return [FullAccess];
			}
