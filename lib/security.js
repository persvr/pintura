...
var AccessError = require("./errors").AccessError,
	first = require("lazy").first,
	bypassSecurity = require("settings").bypassSecurity;
...
exports.DefaultSecurity = function(){
	...
	var Class = require("./persisted").classClass;
	...
	function authenticate(username, password, expires){
		var user = username && security.authenticate(username, password);
	...
	var security = {
		authenticate: function(username, password){
			var user = first(userClass.query("?username=$1", {
				parameters: [username]
			}));
		...
		createUser: function(username, password){
			...
			return security.getUserClass().create({
					username: username,
					password: password
				});
		},
		getAllowedFacets: function(user, request){
			...
			if((user && user.isAdmin) || bypassSecurity){
				return [FullAccess];
			}
		...
		getAuthClass: function(){
			var Class = require("./persisted").Class;
			if(!authClass){
