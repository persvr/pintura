...
exports.DefaultSecurity = function(){
	...
	function authenticate(username, password, expires){
		// FIXME: .getMetadata() returns "undefined"!
		var request = require("perstore/json-rpc").getMetadata().request;
		return when(username && security.authenticate(username, password),
			function (user){
				var session = request.getSession(true, expires);
				request.remoteUser = session.user = user && user.id;
				session.save();
				return user;
			},
			function(){
				...
				if(session.user){
					request.remoteUser = session.user = null;
					session.save();
				}
				return user;
			});
	...
	var security = {
		...
		getAllowedFacets: function(user, request){
			...
			if((settings.admins && user && settings.admins.indexOf(user.id) > -1) || settings.bypassSecurity){
				return [FullAccess];
			}
