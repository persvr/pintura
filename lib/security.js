...
//dir('LOGOUT:', user, security.getAuthClass());
					response = setAuthCookie();
					if (user) {
						return setAuthCookie();
						security.getAuthClass().get('user=$1', {parameters: [user.id]}).then(function(rec){
//dir('LOGOUT1:', rec);
						    //security.getAuthClass().store['delete'](user.id), function(rec){
						});
					}
