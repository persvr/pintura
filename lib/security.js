...
dir('REGISTER?:', user);
				return when(security.getUserModel().put({
						password: password
					}, {id: username}), function(user){
dir('REGISTER!:', user);
					return user;
				});
			});
		...
		getAuthenticationFacet: function(){
			return Restrictive(security.getUserModel(), {
				query: function(){
					throw new AccessError("Can not view user objects1");
				},
				get: function(){
					throw new AccessError("Can not view user objects2");
				},
				post: function(body){
