 ...
 */
var WorkerStore = require("./worker-store").WorkerStore,
	Replicated = require("perstore/store/replicated").Replicated;
...
exports.Notifying = function(store, id){
	...
	var workerStore = WorkerStore(store);
	if(workerStore){
		return Replicated(store, [workerStore]);
	}
	return store;
};
