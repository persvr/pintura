			...
			if(database.nextDatabase){
				...
				return executor({
					openObjectStore: function(storeName){
						...
						return transactions[database.id].openObjectStore(storeName);
					}
				});
			}
		});	
...
exports.transaction = function(executor){
	return rawTransaction(function(transaction){
		...
		var currentTransaction = exports.currentTransaction = {
			openObjectStore: function(storeName){
				//transaction.openObjectStore(storeName); // start the transaction
				...
				store[i] = function(){
					if(currentRawTransaction){
						...
						if(!currentTransactionStore){
							currentTransactionStore = currentRawTransaction.usedStores[store.id] = store;//currentRawTransaction.openObjectStore(store.id);
